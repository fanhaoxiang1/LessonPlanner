    ...
    <div id="app">
        <el-container style="padding-left: 0;">
            ...
            <el-main style="padding-bottom: 0; padding-top: 0; padding-left: 0;">
                <el-container>,
                    <el-main style="height: calc(98vh - 60px); padding: 0 20px 0 70px; margin: 0 20px 0 20px;"
                    ...
                    <div>
                        ...
                        <el-button type="primary" style="margin-top: 10px; margin-bottom: 10px;"
                            @click="isAdvancedEditor = true; removeExtraLines(); clickAdvancedEditor();"
                            plain>使用Markdown编辑器编辑</el-button>
                        <div v-if="isAdvancedEditor">
                            <v-md-editor v-model="lesson_outline_markdown" mode="editable"
                                left-toolbar="undo redo | h bold italic | ul ol code | getSelection openAssistant saveAdvancedEditor"
                                right-toolbar="preview fullscreen" :toolbar="toolbar" height="800px" >
                            </v-md-editor>
                        ...
                        <div v-if="!isAdvancedEditor">
                            ...
                            <div v-for="(section, index_section) in lesson_outline_markdown_split_new"
                                        ...
                                        <div v-if="visibleIndex[0] !== index_section || visibleIndex[1] !== index">
                                            ...
                                            <div v-if="markdown.startsWith('# ')" class="menu-after-h2">
                                                ...
                                                <!-- <el-button :type="buttonType[index_section]" icon="Check"
                                                    ...
                                                    @click="generateLessonPlan(index_section)">{{buttonText[index_section] }}
                                                </el-button> -->
                                                <el-button type="primary" icon="Compass"
                                                    v-if="!ignoreContent[index_section]"
                                                    @click="dialogVisible = index_section;" size="small"
                                                    plain>设置该模块的教学事件
                                                </el-button>
                                                <div style="margin: 20px 0;float: left;">
                                                    <el-popconfirm title="你确定要删除吗？" width="300"
                                                        @confirm="deleteBlock(index_section)">
                                                        <template #reference>

                                                            <el-button type="warning" icon="Delete" size="small"
                                                                style="margin-left: 10px;" plain> 删除该模块内容
                                                            </el-button>
                                                        </template>
                                                    </el-popconfirm>
                                                </div>
                                                ...
                                                <el-dialog :model-value="dialogVisible === index_section" title="选择事件"
                                                    width="600" :modal="false" :align-center="true"
                                                    @open="dialogGetEvents(index_section)">
                                                    <el-checkbox-group v-model="checkedEvents">
                                                        <el-checkbox
                                                            v-for="(event, ind) in ['引起注意', '告知目标', '回忆已有知识', '展示内容', '提供教学指导', '引出练习', '提供反馈', '评估', '知识迁移']"
                                                            :key="ind" :label="event" :value="event">
                                                            {{ event }}
                                                        </el-checkbox>
                                                    </el-checkbox-group>
                                                    ...
                                                    <template #footer>
                                                        <div class="dialog-footer">
                                                            ...
                                                            <el-button type="primary"
                                                                @click="dialogConfirmSelection(index_section)">确认</el-button>
                                                        </div>
                                                    </template>
                                                </el-dialog>
                                            ...
                                            <el-row>
                                                <el-col :span="21">
                                                    <v-md-editor
                                                        ...
                                                        right-toolbar="preview fullscreen" :toolbar="toolbar">
                                                    </v-md-editor>
                                                </el-col>
                                                <el-col :span="3">
                                ...
                                <el-button type='primary' link color="#000000" @click="addBlockBelow(index_section)">
                                    在此处插入新块...</el-button>
                    ...
                    <el-aside width="35%" style="height: 98vh; padding: 0;" v-if="isAssistantOpen">
                        <div>
                            ...
                            <div style="margin-top: 10px">
                                <el-card shadow="hover" body-style="background-color: #e1f3d8;" style="width: 98%; ">
                                    ...
                                    <el-row>
                                        ...
                                        <el-col :span="1">
                                            <!-- Input -->
                                            <el-button type="info" icon="Select" circle
                                                @click="customedRequest(isAdvancedEditor === true ? 2 : 0);"
                                    ...
                                    <div style="display: flex; align-items: center;">
                                        ...
                                        <!-- 这个元素会保持在左边 -->
                                        <el-button type="info" icon="Delete" circle @click="responseContent = ''; stopGenerating = stopGenerating + 1;"
                                            style="margin: 1px 1px 0px 1px; margin-left: 50px;"></el-button>

                                        <!-- 这里使用了自动外边距来推动按钮到右边 -->
                                        <el-button size="small" @click="historyVisible = true" icon="Expand"
                                    ...
                                    <v-md-editor v-model="responseContent" :mode="isAssistantEditable"
                                        ...
                                        :codemirrorConfig="{ lineNumbers: false }" :toolbar="toolbar" height="425px">
                                    </v-md-editor>
                                    ...
                                    <div class="action-buttons">
                                        <el-button size="small" style="margin-bottom: 10px;" type="warning"
                                            @click="stopGenerating = stopGenerating + 1; this.isAssistantEditable = 'edit';">停止生成</el-button>
                                        <!-- <el-button size="small" @click="sendMessage" type="success"
                                            style="margin-bottom: 10px;">继续询问</el-button> -->
                                        <el-button size="small" @click="copyText(responseContent)" type="success"
                                        ...
                                        <el-button size="small" type="primary" @click="confirmAction">关闭助手</el-button>
                                    </div>
                                    <!-- </el-col> -->
                                    <!-- </el-row> -->
                                    <!-- 其他内容 -->
        ...
        <div>
            <el-drawer v-model="historyVisible" title="历史记录" direction="rtl" size="20%" :modal="true"
                :before-close="popoverVisible = false" width="500px">
                <main>
                    <el-popover :visible="popoverVisible" placement="right" width="500px" :offset="40">
                        <div>
                            ...
                            <br />
                            <b>输出：</b><br />
                            <v-md-editor v-model="selectedHistory.response" mode="edit"
                                left-toolbar="undo redo clear| getSelection" right-toolbar="preview"
                                :codemirrorConfig="{ lineNumbers: false }" :toolbar="toolbar" height="425px">
                            </v-md-editor>
...
export default {
    ...
    created() {
        ...
        this.lesson_type = localStorage.getItem('lesson_type');
    ...
    methods: {
        deleteBlock(index) {
            this.lesson_outline_markdown_split_new.splice(index, 1);
            this.ignoreContent.splice(index, 1);
            this.updateMarkdown();
            ElMessage({
                showClose: true,
                message: '已成功删除。',
        ...
        removeExtraLines() {
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                } else if (lines[i].startsWith('## ') && i + 1 < lines.length) {
                    // 如果紧跟着 ## 的是空行，保留一个，跳过后面的所有空行
                    if (lines[i + 1] === '') {
                        i++; // 保留一个空行
                        // 继续跳过后面的所有空行
                        while (i + 1 < lines.length && lines[i + 1] === '') {
            ...
            // 更新全局变量
            this.lesson_outline_markdown = result.join('\n');
        },

        // 在特定行后添加两个空行
        addExtraLines() {
            this.lesson_outline_markdown = this.lesson_outline_markdown.split('\n').reduce((acc, line, index) => {
                // 对于#和##标题，前后都添加两个换行符
                if (line.startsWith('# ') || line.startsWith('## ')) {
                    // 在标题前添加两个换行符，除非已经在文档开头
                    if (index !== 0) {
                        acc.push('\n', '\n');
                    }
                    acc.push(line);
            ...
            // 由于连续的添加可能产生多余的空行，最后对连续超过四个以上的换行进行简单的替换
            this.lesson_outline_markdown = this.lesson_outline_markdown.replace(/\n{4,}/g, '\n\n\n');
        },
        dialogConfirmSelection(index) {
            this.dialogVisible = -1;
            console.log(this.checkedEvents);
            // 排除不在选项内的事件
            this.checkedEvents = this.checkedEvents.filter(event => ['引起注意', '告知目标', '回忆已有知识', '展示内容', '提供教学指导', '引出练习', '提供反馈', '评估', '知识迁移'].includes(event));
            this.lesson_outline_markdown_split_new[index][1] = this.lesson_outline_markdown_split_new[index][1].substring(0, 3) + this.checkedEvents.join(' & ');
            this.updateMarkdown();
        ...
        dialogGetEvents(index) {
            let eventsLine = this.lesson_outline_markdown_split_new[index][1];
            this.checkedEvents = eventsLine.substring(3).split(' & ').map(x => x.trim());
        },
        ...
        customedRequest(x) {
            ...
            // this.messages.push({
            //     type: 'request',
            //     content: `请你为${this.lesson_name}的${this.lesson_theme}生成部分教案大纲，要求之前已经告诉过你了。`
            // });
            if (x !== 2) {
                this.messages.push({
                    type: 'response',
                ...
                if (this.selectedText === '') {
                    ...
                    this.messages.push({
                        type: 'response',
                        content: `您在为${this.lesson_type}的同学设计${this.lesson_name}的${this.lesson_theme}教案时，有任何问题，可以随时问我！`
                    });
                }
                else {
                    this.messages.push({
            ...
            let prompt_to_send = `我在为${this.lesson_type}的同学设计${this.lesson_name}的${this.lesson_theme}教案。${this.selectedText}，\n，我想知道或者想说:` + this.askPrompt;
            let prompt_to_save = this.askPrompt;
            if (x === 1) {
                prompt_to_send = `您在为${this.lesson_type}的同学设计${this.lesson_name}-${this.lesson_theme}的教案`+"我想要重新生成这一部分的课程大纲或是课程内容，我希望内容有所变化，让我有另外一种选择。你只需要返回我重新生成的内容即可，不需要任何前后缀。";
                prompt_to_save = "重新生成";
            }
            this.stopGenerating += 1;
            let stopGenLocal = this.stopGenerating;
            // 避免为空
            fetchStream(
                'http://localhost:8000/lesson-edit/get-button-response/',
                {
                    ...
                    selection: this.selectedText === '' ? '1' : this.selectedText,
                    textArea: this.textAreaNow === '' ? '1' : this.textAreaNow,
                ...
                (chunk) => {
                    // 成功接收到数据块时的处理逻辑
                    // 根据你的需求处理 chunk
                    if (this.stopGenerating !== stopGenLocal) {
                        return;
                    }
                    else {
                        this.isAssistantEditable = 'preview';
                ...
                () => {
                    this.add_history(prompt_to_save, this.responseContent);
                    // this.messages;
        ...
        addBlockBelow(index) {
            this.lesson_outline_markdown_split_new.splice(index + 1, 0, [["# 请输入标题"], ["## 请选择事件"], ["### 请键入分钟"], [" "]]);
            this.updateMarkdown();
        },
        updateMarkdown() {
            const markdownLines = [];
        ...
        saveFromAdvancedEditor(text) {
            this.lesson_outline_markdown = text;
            this.addExtraLines();
            this.isAdvancedEditor = false;
            alert('test');
        },
        ...
        async sendMessage() {
            ...
            // 先将当前部分给push到system的prompt中：
            const sys_prompt = `我正在为"${this.lesson_type}"的学生设计"${this.lesson_name}"-"${this.lesson_theme}"的教案，现在在这个环` +
                `节我遇到一些问题想要你提供建议，你只需要给我答案，不需要任何其他的话语。这是一些上下文，可能对你有用：${this.selectedText}。`
        ...
        generateLessonPlan(sectionIndex) {
            ...
            let result = sectionLines.slice(0, 2);
            // 清除第三个及其以后的内容
            if (sectionLines[2]) {
                // 将sectionLines[2]拆分为行
                let lines = sectionLines[2].split('\n');
                // 检查是否有至少一行
                if (lines.length > 0) {
                    // 取第一行并添加到结果数组
                    result.push(lines[0]);
            ...
            this.lesson_outline_markdown_split_new[sectionIndex] = result;
